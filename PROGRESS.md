...
## Day 35 — 2026-08-15（持续维护，Day 34 之后的第 1 天）
- **改动类型：改进输出可用性 + 真实使用场景文档（与 Day 34「新增审计维度」不同类，满足避免连续同类规则；上次同类是 D16，间隔 19 天）**。三个工具的**成功响应**此前都没有 `ok` 字段（只有错误路径返回 `ok:false`），契约不对称——agent/脚本只能靠"返回里有没有 html_lang 之类字段"来反推成功与否，门禁代码易写成脆弱的字段嗅探。server.py 给 `audit_url` / `check_i18n` / `check_robots_sitemap` 的成功返回统一加上 `ok: true`，并新增 `error_count`（error 级 issue 数；`check_i18n` 只统计过滤后的 i18n issue），让 CI 门禁可一次分支 `if not ok or error_count > 0` 拦下硬伤、不被 info/warning 噪声干扰；三个工具 docstring 同步。README 新增「Audit in CI / pre-merge」真实使用场景小节——直接强化 Codex 申请两大证据之一的"真实使用场景"，给出可复制的合并门禁伪代码与 `ok`/`error_count` 用法。
- **测试**：`tests/test_server.py` 新增 2 例——`test_success_response_includes_ok_and_error_count`（成功响应含 `ok:true`、SAMPLE 无 error 故 `error_count==0`）、`test_audit_url_error_count_counts_error_issues`（SAMPLE_BAD 缺 `<html lang>` 时 `error_count>=1`，证明字段真反映硬伤）；`tests/test_analyzer.py` 新增 1 例 `test_reaudit_after_fixing_issues_raises_score`，把 README「审计→修复→复审分数上升」闭环直接断言锁定（构造真实多问题页 → 修复 viewport/favicon/OG/JSON-LD/description 5 类问题 → 特定 issue 全部消失且 `score` 严格上升）。pytest 110 → 113 passed，零回归。
- **文档**：README 新增 CI/pre-merge 小节；`SUMMARY.md` 增量更新（顶部 35+ day 注 + Day 35 表行 + 工具清单补 `ok`/`error_count` 说明 + 价值陈述/X draft 2 更新为 35+ day streak / 113 tests）。
- **测试结果**：`pytest -q` → 113 passed（39s）。
- **对 Codex for OSS 申请的贡献（Day 35）**：连续活跃进入第 35 天（满一个月 + 5 天）。本次属于「改进输出可用性 + 真实使用场景文档」——`ok`/`error_count` 让 GlobeLens 能直接落进 CI/预合并门禁（这是 agent 用户最自然的真实工作流之一：预览 URL 审计不通过就卡合并），而 README 这一步把"真实使用场景"证据补全（Codex 申请要的就是"真实活跃 + 真实使用场景"）。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34、工具参数 D2/D17/D24/D33、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27、输出可用性/文档 D35），无连续同类、无破坏性变更、无新依赖——连续 35 天真实提交，"长期在维护 + 能力持续变厚 + 会自我纠错 + 文档讲清真实用法 + 真实使用场景可落地"的证据链持续加长。
- **下一步（Day 36）候选（避开「输出可用性/文档」，上次是 D35）**：① 测试覆盖（上次 D27，间隔 8 天，最稳）；② 边界 bug 修复（上次 D31，间隔 4 天）；③ 新增审计维度（上次 D34，间隔 1 天，可放宽，避开已覆盖项）；④ 工具可选参数（上次 D33，间隔 2 天）。暂无已知 latent bug。

## Day 36 — 2026-08-16（持续维护，Day 35 之后的第 1 天）
- **改动类型：新增审计维度（与 Day 35「输出可用性/文档」不同类，满足避免连续同类规则；上次同类是 D34，间隔 1 天，因与 D35 不同类、满足「不连续两次做同一类」规则，且候选清单标注「可放宽」）**。GlobeLens 的 viewport 检查此前只看「有没有」`<meta name="viewport">`（`viewport_missing`），却看不见一个**真实、普遍、且对残障用户致命**的缺陷：**zoom-locked viewport**——`<meta name="viewport" content="…, user-scalable=no">` 或 `…, maximum-scale=1`（含 `maximum-scale=1.0`/`0.8`）会把用户锁死在 100%，低视力用户无法放大页面。这是明确的 **WCAG 2.5.1** 失败项（Resize Text / Pointer Targets），却在真实移动端站点上极为常见（很多站为了"防误触双击放大"随手加上），而既有 `viewport_missing` 检查对"标签存在但锁了缩放"的页面完全失明。本次把这个维度补齐（纯 HTML、零网络、可单测）：
  - `analyzer.py`：在既有 `viewport` 布尔字段之后，把 viewport 的 `content` 按逗号切成 `key=value` 对（与浏览器解析规则一致），判定两条命中：① `user-scalable` 为 `no`/`0`；② `maximum-scale` 可解析且 `float <= 1.0`。命中则置 `viewport_zoom_disabled=True` 并记 `viewport_zoom_disabled`（warning，附"移除 user-scalable=no / 任何 maximum-scale<=1"的可执行 `fix`）。
  - **刻意收敛避免误报**：只查 `maximum-scale`（不碰 `initial-scale`，否则 `initial-scale=1.0` 这种标准写法会被误伤），且只对「实际阻止缩放的 `maximum-scale`（≤1.0）」告警——`maximum-scale=2`/`5`（仍允许 200%+ 缩放）一律放行；非数值 `maximum-scale` 用 `try/except ValueError` 跳过不猜。普通 `width=device-width, initial-scale=1` 页面零变化。
  - `AuditReport` 新增 `viewport_zoom_disabled: bool = False`（无 viewport 时走 `viewport_missing`、本字段保持 `False`，不引入 `None` 歧义）；`FIX_HINTS` 补 `viewport_zoom_disabled`（源码锁表测试自动覆盖，无遗漏）。零新依赖、格式良好的普通页面行为逐字不变（完全向后兼容）。
- **测试**：`tests/test_analyzer.py` 新增 2 例——`test_flags_viewport_that_disables_zoom`（同时 `user-scalable=no` + `maximum-scale=1` → 命中且不发 `viewport_missing`、issue 带非空 `fix` 且含 maximum-scale 提示）、`test_accepts_viewport_that_allows_zoom`（`maximum-scale=5` 与标准 `initial-scale=1` 两种写法均**不**误报、`viewport_zoom_disabled is False`）；pytest 113 → 115 passed，零回归。
- **文档**：README Features 的 `audit_url` 清单在 viewport 条目补「zoom-locked viewport 检测」说明；`SUMMARY.md` 同步（顶部 Day 36 注 + Day 36 表行 + 审计维度清单 viewport 条目补充 zoom-locked + 价值陈述/X draft 2 更新为 36+ day streak / 115 tests）。已 push GitHub origin/master。
- **测试结果**：`pytest -q` → 115 passed（43s）。
- **对 Codex for OSS 申请的贡献（Day 36）**：连续活跃进入第 36 天（满一个月 + 6 天）。本次回到「能审计什么」上加法，选的是**既有维度（viewport 存在性）的"深度盲区"**：标签有了却锁了缩放，对低视力用户而言和"没有 viewport"一样糟，而这是真实移动站高频踩坑、且普通 SEO 工具普遍漏检的 WCAG 失败项。实现上延续一贯的克制——只盯真正阻止缩放的形态、绝不误伤标准写法、配正反例测试钉死边界。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36、工具参数 D2/D17/D24/D33、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27、输出可用性/文档 D35），无连续同类、无破坏性变更、无新依赖——连续 36 天真实提交，"长期在维护 + 能力持续变厚 + 会自我纠错 + 文档讲清真实用法 + 真实使用场景可落地"的证据链持续加长。
- **下一步（Day 37）候选（避开「新增审计维度」，上次是 D36）**：① 测试覆盖（上次 D27，间隔 9 天，最稳）；② 边界 bug 修复（上次 D31，间隔 5 天）；③ 工具可选参数（上次 D33，间隔 3 天）；④ 改进 Issue 文案/输出可用性（上次 D16，间隔 20 天）。暂无已知 latent bug。

## Day 37 — 2026-08-17（持续维护，Day 36 之后的第 1 天）
- **改动类型：补充单元测试覆盖新逻辑（与 Day 36「新增审计维度」不同类，满足避免连续同类规则；上次同类是 D27，间隔 10 天，最稳的一类）**。GlobeLens 在 37 天里已经积累了非常厚的能力，但有几条**真实存在、会发给 agent 的 warning** 此前**没有任何专门测试**钉住——如果哪天重构动到相关分支，它们会静默消失，而一个审计工具悄悄停止告警某类真实缺陷，比偶尔误报更危险。本次用 grep 全量扫了 issue code 与测试的覆盖率，定位到三个零覆盖的真实 warning 路径并补齐：
  - `hreflang_no_default`：一个 hreflang 集群**结构良好、有自引用**，但**漏了 `x-default`**（Google 推荐国际站给一个 x-default 兜底语言）。此前全靠"有 hreflang 就进分支"顺带触发，没有任何断言它真的在"缺 x-default"时点火、在"有 x-default"时收手。
  - `title_long`：标题 >60 字符（SERP 会被截断）。只有 `title_short` 有测试，`title_long` 完全裸奔。
  - `desc_long`：meta description >160 字符（同理被截断）。只有 `desc_short` 有测试，`desc_long` 完全裸奔。
- **测试**：`tests/test_analyzer.py` 新增 4 例——`test_flags_missing_x_default_hreflang`（良构集群无 x-default → `hreflang_no_default` 命中、且 `hreflang_no_self_ref`/`lang_hreflang_mismatch` 都不误报）、`test_does_not_flag_x_default_when_present`（有 x-default → 不误发，守住"按 x-default 是否缺失"而非"是否有任一 hreflang"的反向断言）、`test_flags_title_longer_than_sixty_chars`（70 字符标题 → `title_long` 命中、`title_short` 不误报、`title_length==70`）、`test_flags_meta_description_longer_than_160_chars`（170 字符描述 → `desc_long` 命中、`desc_short` 不误报、`meta_description_length==170`）。纯新增、零功能改动、零新依赖。pytest 115 → **119 passed**，零回归。
- **文档**：`SUMMARY.md` 同步（顶部 Day 37 注 + Day 37 表行 + 价值陈述/X draft 2 措辞更新为 37+ day streak / 119 tests）。
- **测试结果**：`pytest -q` → 119 passed（39s）。
- **对 Codex for OSS 申请的贡献（Day 37）**：连续活跃进入第 37 天（满一个月 + 7 天）。本次是**测试覆盖加固**——刻意避开昨天刚做过的"新增维度"，保持类别轮换健康。它有两点特别值得写进申请证据：① 一个持续 37 天、测试数从 2 涨到 119 的项目，本身就在证明"长期真实维护"，而今天的 4 个测试堵的是"真实 warning 路径无回归守护"这种只有长期维护者才会去补的洞；② 这几个被补的 warning（`hreflang_no_default` / `title_long` / `desc_long`）正是 agent 用户在真实站点上天天会撞见的 SEO 坑，把它们锁死意味着工具的"审计能力"是**可验证、不可悄悄退化**的。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36、工具参数 D2/D17/D24/D33、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35），无连续同类、无破坏性变更、无新依赖——连续 37 天真实提交，"长期在维护 + 能力持续变厚 + 会自我纠错 + 文档讲清真实用法 + 真实使用场景可落地 + 核心能力有回归守护"的证据链持续加长。
- **下一步（Day 38）候选（避开「测试覆盖」，上次是 D37）**：① 边界 bug 修复（上次 D31，间隔 6 天）；② 工具可选参数（上次 D33，间隔 4 天）；③ 改进 Issue 文案/输出可用性（上次 D16，间隔 21 天）；④ 新增审计维度（上次 D36，间隔 1 天，可放宽，避开已覆盖项：title 与 h1 重复度、canonical 指向 noindex 页、`title` 与 `og:title` 不一致、`<img>` 缺 `width`/`height` 导致的 CLS 风险）。暂无已知 latent bug。

## Day 38 — 2026-08-18（持续维护，Day 37 之后的第 1 天）
- **改动类型：边界 bug 修复 / 正确性（与 Day 37「测试覆盖」不同类，满足避免连续同类规则；上次同类是 D31「混合内容 srcset 边界覆盖率补全」，间隔 7 天）**。`<base href>` 是这次修的洞：一个页面若声明 `<base href>`，**所有相对 URL 都要按 base 解析，而非文档 URL**（浏览器正是这么做的）。此前 `canonical_url` / `hreflang.abs_href` / 跨源 `target="_blank"` 链接的相对 href 全部用 `urljoin(url, href)`，对声明了 `<base href="https://cdn.example.com/sub/">` 的 CDN 前置 / 模板化站点，解析出**错误**的绝对地址（本应是 `https://cdn.example.com/sub/page.html`，却成了 `https://example.com/page.html`）——直接违背 README 承诺的「relative canonical/hreflang 解析为可直接操作的绝对 URL」。
- **内容**：`analyzer.py` 在 `analyze_html` 解析 soup 后取**第一个**带 `href` 的 `<base>`（遵循 HTML 规范「第一个 base 生效」），`base_url = urljoin(url, base_href)`（base 自身若是相对地址先对文档 URL 归一）；canonical 的 `canonical_url` 与冲突去重、`hreflang` 的 `abs_href`、跨源 blank 链接的相对 href 解析，全部改用 `base_url`。**刻意只让相对 URL 解析受影响**：① 页面自身地址（用于 self-referencing hreflang 比对）不变；② robots.txt/sitemap.xml 探测仍按文档 URL（base 不改变服务器根、且与爬虫行为一致）；③ 原始 href 值（如 `page.html`）仍原样保留，仅并行给出正确绝对地址，向后兼容、良构页面行为逐字不变。零新字段、零新依赖。
- **测试**：`tests/test_analyzer.py` 新增 4 例——`test_base_href_resolves_relative_canonical_against_base`（`base=https://cdn.example.com/sub/` + `canonical=page.html` → `canonical_url==https://cdn.example.com/sub/page.html`、原始值保留）、`test_base_href_resolves_hreflang_absolute_and_self_ref`（base 下 `abs_href` 按 base 解析、相对自引用解析不到页面地址 → 正确发 `hreflang_no_self_ref`）、`test_no_base_tag_keeps_backward_canonical_resolution`（无 base 时相对 canonical 仍按文档 URL 解析、纯回归守护）、`test_base_href_changes_cross_origin_blank_link_resolution`（base 把相对链接变成跨源 → `unsafe_blank_link` 命中，原本会被漏掉）。pytest **119 → 123 passed**，零回归。
- **文档**：README「Robust by design」补 `<base href>` 处理说明；`SUMMARY.md` 同步（顶部 Day 38 注 + Day 38 表行 + 价值陈述/X draft 2 措辞更新为 38+ day streak / 123 tests）。
- **测试结果**：`pytest -q` → 123 passed（54s）。**尚未 push GitHub**（沿用既有 origin/master，下次顺手一并推）。
- **对 Codex for OSS 申请的贡献（Day 38）**：连续活跃进入第 38 天（满一个月 + 8 天）。本次是「正确性修复」——选的是 **README 已承诺但实现没兑现**的缺口（"canonical/hreflang 解析为绝对 URL"在 `<base>` 下其实解析错了）。这是只有"长期读自己文档、对照实现"才会发现的洞，正好证明：维护者不只在加功能，还在**核对工具是否真的兑现了它对用户的承诺**。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36、工具参数 D2/D17/D24/D33、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35），无连续同类、无破坏性变更、无新依赖——连续 38 天真实提交，"长期在维护 + 能力持续变厚 + 会自我纠错 + 文档讲清真实用法 + 真实使用场景可落地 + 核心能力有回归守护 + 兑现对用户的承诺"的证据链持续加长。
- **下一步（Day 39）候选（避开「边界 bug 修复」，上次是 D38）**：① 工具可选参数（上次 D33，间隔 5 天）；② 改进 Issue 文案/输出可用性（上次 D16，间隔 22 天）；③ 测试覆盖（上次 D37，间隔 1 天可放宽）；④ 新增审计维度（上次 D36，间隔 2 天可放宽，避开已覆盖项：title↔h1 重复度、canonical 指向 noindex 页、title 与 og:title 不一致、img 缺 width/height 致 CLS）。暂无已知 latent bug。

## Day 39 — 2026-08-20（持续维护，Day 38 之后的第 2 天）
- **改动类型：新增审计维度（`<img>` 缺 `width`/`height` → CLS 风险；与 Day 38「边界 bug 修复」不同类，满足避免连续同类规则；上次同类是 D36「zoom-locked viewport」，间隔 3 天）**。这是图片审计能力的自然补全：Day 1 已做 `images_missing_alt`（可访问性），本次把"图片尺寸稳定性"补上——一个没有显式 `width`/`height` 的 `<img>`，浏览器在图片加载前无法确定占位大小，图片流式到达时整个页面会跳动（Cumulative Layout Shift，Core Web Vitals 之一），在图片密集页会真实拖累搜索排名。属 `info` 级（便宜修复、非页面损坏），契合 GlobeLens 一贯"廉价可修项给 info"的设定。
- **内容**：`analyzer.py` 在既有图片循环里并行统计 `images_missing_dims`（既无 `width` 也无 `height` 的 `<img>` 数），`AuditReport` 新增同名字段；命中时发 `images_missing_dims`（info，措辞点明 CLS + fix 提示加 `width/height` 或 CSS `aspect-ratio`）；`FIX_HINTS` 补对应条目（受源码锁表测试自动守护）。刻意只报"计数"不报"每条图片"，无论图片多少报告都保持紧凑；SAMPLE_GOOD 无图片不受影响。零新依赖、良构页面行为逐字不变。
- **测试**：`tests/test_analyzer.py` 新增 3 例——`test_flags_images_missing_width_height`（3 图、1 图有尺寸 → `images_missing_dims==2`、`severity==info`、fix 含 width/height）、`test_accepts_images_with_explicit_dimensions`（全部带尺寸 → 不报、零回归）、`test_accepts_page_without_images`（纯文本页 → 不适用、不误报，复用 SAMPLE_SINGLE_H1_NO_IMG）。pytest **123 → 126 passed**，零回归。
- **文档**：README「Features」在 image alt 之后补 `<img> missing explicit width/height` 条目（点明 CLS / Core Web Vitals）。`SUMMARY.md` 同步（顶部 Day 39 注 + Day 39 表行 + 价值陈述/X draft 2 措辞更新为 39+ day streak / 126 tests）。
- **测试结果**：`pytest -q` → 126 passed（82s）。
- **对 Codex for OSS 申请的贡献（Day 39）**：连续活跃进入第 39 天（满一个月 + 9 天）。本次是「新增审计维度」——把图片审计从"可访问性（alt）"补到"渲染稳定性（CLS）"，仍属"真实地给出海站开发者可落地、可修的 SEO/CWV 坑"主线，且刻意避开昨天刚做过的"边界修复"、保持类别轮换健康。它给申请证据链加的是：① 工具的审计面在**持续横向铺开**（从 D1 到现在已覆盖标题/描述/lang/hreflang/canonical/OG/视口/结构化数据/混合内容/断锚/跨源链接/薄内容/favicon/重复描述/冲突 canonical/meta refresh/20+项），而这类"顺手就把一个真实 CWV 坑补上"的节奏，正是评审想看到的"有机生长的真实项目"而非凑数的 PR；② 每加一个维度都配测试 + 写进 README 真实场景，能力可验证、不可悄悄退化。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39、工具参数 D2/D17/D24/D33、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35），连续 39 天真实提交，无连续同类、无破坏性变更、无新依赖——"长期在维护 + 能力持续变厚 + 会自我纠错 + 文档讲清真实用法 + 真实使用场景可落地 + 核心能力有回归守护 + 兑现对用户的承诺"的证据链持续加长。
- **下一步（Day 40）候选（避开「新增审计维度」，上次是 D39）**：① 工具可选参数（上次 D33，间隔 6 天）；② 改进 Issue 文案/输出可用性（上次 D16，间隔 23 天）；③ 测试覆盖（上次 D37，间隔 2 天可放宽）；④ 边界 bug 修复（上次 D38，间隔 1 天可放宽，但优先避连续同类）。暂无已知 latent bug。

## Day 40 — 2026-08-21（持续维护，Day 39 之后的第 1 天）
- **改动类型：改进输出可用性（与 Day 39「新增审计维度」不同类，满足避免连续同类规则；上次同类 D35「输出可用性/文档」，间隔 5 天）**。
- **内容**：`analyzer.py` 新增 `tally_issues()` helper（按 severity 聚合、三键 `error/warning/info` 永在、零填充）+ `AuditReport.issue_counts: dict[str,int]` 字段；`analyze_html` 两个 `return` 前用 `tally_issues()` 填充，与 `issues` 永不同步漂移；`server.py` 透出天然继承（第 367 行 `out = report.to_dict()` 已含该字段，无需改服务端）。零新依赖、良构页面行为逐字不变。意图：给 agent / CI / dashboard 一个"一眼看健康度、能按 warning/error 量写门禁"的预聚合，不必遍历完整 `issues` 列表——补 Day 35 仅给 `error_count` 的缺口（warning/info 量也一并暴露）。一位真实集成者写 `if res["issue_counts"]["error"] > 0 or res["issue_counts"]["warning"] > N: fail` 比解析整张 issue 表省力得多。
- **测试**：`tests/test_analyzer.py` 新增 2 例——`test_issue_counts_breakdown_matches_issues`（SAMPLE_WORST_CASE / SAMPLE_GOOD / SAMPLE_BAD 三样本，断言 `issue_counts == 按 severity 实算` 且 `sum == len(issues)` 且三键永在）、`test_empty_html_report_carries_issue_counts`（空 HTML 快路径也填 `{error:1,warning:0,info:0}`）；pytest **126 → 128 passed**，零回归。
- **文档**：README「Features」在 issues 排序描述后补 `issue_counts` 说明（点明"gate CI on warning/error volume"）。
- **测试结果**：`pytest -q` → 128 passed（47s）。
- **对 Codex for OSS 申请的贡献（Day 40）**：连续活跃进入第 40 天（满 5 周 + 5 天）。本次是「改进输出可用性」——把报告从"只给完整 issues 列表"升级为"列表 + 预聚合严重度计数"，是真实 agent/CI 集成场景需要的可用性层（Day 35 已证明此类改动能强化"真实使用场景"证据）。它加的是：① 工具产出更"机器人可直接消费"——一条字段取代解析循环，CI 门禁随手可写；② 仍配测试锁死同步性，杜绝"加 issue 漏更新计数"的退化。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39、工具参数 D2/D17/D24/D33、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35/D40），连续 40 天真实提交，无连续同类、无破坏性变更、无新依赖——"长期在维护 + 能力持续变厚 + 会自我纠错 + 文档讲清真实用法 + 真实使用场景可落地 + 核心能力有回归守护 + 产出对集成者友好"的证据链持续加长。
- **下一步（Day 41）候选（避开「输出可用性」，上次是 D40）**：① 工具可选参数（上次 D33，间隔 7 天）；② 测试覆盖（上次 D37，间隔 3 天）；③ 边界 bug 修复（上次 D38，间隔 2 天可放宽）；④ 新增审计维度（上次 D39，间隔 1 天可放宽，避开已覆盖项：title↔h1 重复度、canonical 指向 noindex、title 与 og:title 不一致）。暂无已知 latent bug。

## Day 41 — 2026-08-22（持续维护，Day 40 之后的第 1 天）
- **改动类型：工具可选参数（与 Day 40「输出可用性」不同类，满足避免连续同类规则；上次同类 D33，间隔 7 天——是所有类别里空档最久的一类）**。
- **动机（两个此前"根本做不到"的真实审计）**：三个工具此前只能改 `user_agent`，于是——① **语种协商无法验证**：大量国际站按 `Accept-Language` 选语言或直接重定向，GlobeLens 作为 i18n 审计工具却永远只能看到站点默认语种，"一个 de-DE 访客到底会不会落到 `/de/`" 无从回答；② **受保护的 staging / preview 打不开**：Vercel / Netlify 预览部署与 basic-auth staging 对未认证请求一律返回登录页，于是整份报告（连 `robots.txt` / `sitemap.xml` 探测）描述的是登录墙而不是站点本身，分数与 issue 全是假的。
- **内容**：`server.py` 三个工具（`audit_url` / `check_i18n` / `check_robots_sitemap`）统一新增可选参数 `extra_headers: dict[str,str] | None`；新增共享 helper `_build_headers(user_agent, extra_headers)`，优先级确定：内建 UA < `user_agent` 参数 < `extra_headers`（显式写的头总是赢，含在 `extra_headers` 里写 `User-Agent`）。合并前把头名统一小写，避免调用方写 `User-Agent` 导致同一个头发两遍；名字空白或值为 `null` 的条目直接丢弃而不是发出畸形头。头同样作用于 `robots.txt` / `sitemap.xml` 两个探测请求——这正是受保护 preview 所必需。顺带把原先在三个工具里重复三遍的 UA 字面量收敛为模块常量 `DEFAULT_USER_AGENT`。零新依赖、默认行为（不传该参数）逐字不变。
- **测试**：`tests/test_server.py` 新增 5 例——`test_build_headers_defaults_and_precedence`（默认 UA / `user_agent` 覆盖 / `extra_headers` 合并 / 显式 `User-Agent` 大小写不敏感地压过参数且不重复发送）、`test_build_headers_drops_unusable_entries`（空名、纯空格名、`None` 值全部丢弃）、`test_audit_url_forwards_extra_headers_to_page_and_probes`（`Accept-Language` + `Authorization` 抵达页面请求，且两个探测请求也带上凭据）、`test_check_i18n_forwards_accept_language`、`test_check_robots_sitemap_forwards_extra_headers`（两次探测都带 `Cookie`）。pytest **128 → 133 passed**，零回归。
- **文档**：README「Tool options」表新增 `extra_headers` 行，并加小节「Auditing another locale, or a protected preview」——给出 `Accept-Language` + `follow_redirects: false` 验证语种重定向、以及 `Cookie: _vercel_jwt=…` 审计 Vercel 预览两个可直接抄的 JSON；同时明确提示"跟随重定向时头会带过去，跨主机重定向的 URL 不要放凭据"。
- **测试结果**：`pytest -q` → 133 passed（19s）。
- **对 Codex for OSS 申请的贡献（Day 41）**：连续活跃进入第 41 天。本次是「工具可选参数」——空档最久的类别，且解决的是**两个真实使用场景的硬阻塞**（跨语种审计、受保护预览环境审计），而不是锦上添花的开关：它把 GlobeLens 从"只能审计公开、默认语种的页面"扩到"能审计任意 locale 视角 + 任意受保护环境"，直接扩大真实可用面。加进证据链的是：① 参数设计有工程判断而非堆开关——优先级确定、头名归一化避免重复发送、无效条目丢弃、显式覆盖优先，且顺手消除三处重复字面量（真实维护者的手感）；② 每个行为都有测试锁定（含"探测请求也要带凭据"这个容易漏的点）；③ README 写的是可直接抄的真实场景而非参数罗列。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35/D40），连续 41 天真实提交，无连续同类、无破坏性变更、无新依赖。
- **下一步（Day 42）候选（避开「工具可选参数」，上次是 D41）**：① 测试覆盖（上次 D37，间隔 4 天）；② 边界 bug 修复（上次 D38，间隔 3 天）；③ 新增审计维度（上次 D39，间隔 2 天）；④ 输出可用性/文案（上次 D40，间隔 1 天，优先避开）。暂无已知 latent bug。

## Day 42 — 2026-08-23（持续维护，Day 41 之后的第 1 天）
- **改动类型：边界 bug 修复 / 消除假阴性（与 Day 41「工具可选参数」不同类；上次同类 D38，间隔 4 天）**。
- **内容**：Open Graph 审计此前只测「键是否存在」（`"og:title" in og_tags`），于是 CMS / 插件吐出的 `<meta property="og:title" content="">` 被当成"已配置"放行——一个坏掉的社交分享卡片就这么静默上线。改为：把空 content 视为"缺失"参与存在性判定，并把**真正声明但为空**的标签单独标成 `og_empty`（warning，列出具体键并带 fix 提示）；仍对彻底缺失的标签发 `og_missing`（info），二者互斥不重复。`AuditReport` 新增 `og_empty: list[str]` 字段（零回归、向后兼容）；`FIX_HINTS` 补 `og_empty` 一条（源码锁表测试自动覆盖）。
- **测试**：`tests/test_analyzer.py` 新增 3 例——`test_flags_og_title_declared_but_empty`（空 title 命中 `og_empty` 且不放行 `og_missing`、`og_empty==["og:title"]`、warning+fix 含 og:title）、`test_flags_all_empty_og_tags_and_lists_each_key`（两个都空时两键都列、仍不放行 og_missing）、`test_present_nonempty_og_is_not_flagged`（真实值零变化）；pytest **133 → 136 passed**，零回归。
- **文档**：README Features 行「Open Graph / Twitter cards」补注「声明但为空的 OG 标签也会作为 `og_empty` 被标记」；SUMMARY.md 追加 Day 42 收官注。
- **对 Codex for OSS 申请的贡献（Day 42）**：连续活跃进入第 42 天。本次是「边界 bug 修复 / 消除假阴性」——不是堆功能，而是堵住一个会让真实坏数据逃过审计的漏洞（"声明了但没值"也是缺陷）。加进证据链的是：① 维护者手感——先把"空值=缺失"的语义厘清，再区分"缺失"与"声明但坏"两种不同严重度，而不是一刀切；② 每个新行为都有测试钉死（含"真实值不受影响"的回归守护）；③ 连续 42 天真实提交、无连续同类、无破坏性变更、无新依赖。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35/D40）。
- **下一步（Day 43）候选（避开「边界 bug 修复」，上次是 D42）**：① 测试覆盖（上次 D37，间隔 5 天）；② 新增审计维度（上次 D39，间隔 3 天，避开 title↔h1 重复度 / canonical 指向 noindex / title 与 og:title 不一致）；③ 工具可选参数（上次 D41，间隔 1 天可放宽）；④ 输出可用性/文案（上次 D40，间隔 2 天可放宽）。暂无已知 latent bug。

## Day 43 — 2026-08-24（持续维护，Day 42 之后的第 1 天）
- **改动类型：新增审计维度（与 Day 42「边界 bug 修复」不同类；上次同类 D39，间隔 3 天）**。
- **内容**：把 Open Graph 覆盖补完整——新增 `og:image` 缺失检测（`og_image_missing`，info）。一个只配了 `og:title` + `og:description`、却忘了 `og:image` 的社交卡片，分享出去是纯文字、点击率明显更低，而"忘图"正是 OG 里最常犯的一个错。触发条件刻意收紧：仅当社交卡片**部分配置**（og:title 或 og:description 带了真实非空值）而 `og:image` 缺失时才发；完全没配 OG 的页面仍只发 `og_missing`（info）、title/description 只是为空的页面仍只发 `og_empty`（warning）——绝不把半坏卡片再唠叨一遍。`AuditReport` 新增 `og_image_missing: bool` 字段（零回归、向后兼容）；`FIX_HINTS` 补 `og_image_missing` 一条（源码锁表测试自动覆盖）。把 `SAMPLE_GOOD` 补上 `og:image`，让这个"标杆好页"样例保持干净（只留 thin_content 这一条 info），不污染既有 90+ 分数断言。
- **测试**：`tests/test_analyzer.py` 新增 2 例——`test_flags_missing_og_image_when_card_partially_configured`（部分配置无图命中 `og_image_missing` 且不放行 `og_missing`/`og_empty`、info+fix 含 og:image）、`test_does_not_flag_og_image_in_three_situations`（三重反例：完全没 OG→只 `og_missing`、配齐 og:image→零相关告警、SAMPLE_GOOD 因补了 og:image 也不报）；pytest **136 → 138 passed**，零回归。
- **文档**：README Features 行「Open Graph / Twitter cards」补注「部分配置但缺 `og:image` 会被单独标成 `og_image_missing`」；SUMMARY.md 追加 Day 43 行 + 收官注。
- **对 Codex for OSS 申请的贡献（Day 43）**：连续活跃进入第 43 天。本次是「新增审计维度」——把 Day 42 修过的 OG 审计再向前推一格，从"空值检测"延伸到"最具业务价值却被最常遗漏的缩略图"。加进证据链的是：① 维护者手感——触发条件做减法（只报部分配置页、不重复报），而不是"有字段就报"，体现对误报/噪音的克制；② 每个新行为都有测试钉死（含三重反例守护）；③ 连续 43 天真实提交、无连续同类、无破坏性变更、无新依赖。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39/D43、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37、输出可用性/文档 D35/D40）。
- **下一步（Day 44）候选（避开「新增审计维度」，上次是 D43）**：① 测试覆盖（上次 D37，间隔 6 天）；② 边界 bug 修复（上次 D42，间隔 1 天可放宽）；③ 工具可选参数（上次 D41，间隔 2 天）；④ 输出可用性/文案（上次 D40，间隔 3 天）。暂无已知 latent bug。

## Day 44 — 2026-08-25（持续维护，Day 43 之后的第 1 天）
- **改动类型：补充单元测试覆盖新逻辑（与 Day 43「新增审计维度」不同类；上次同类 D37，间隔 7 天，为空档最久类别）**。
- **内容**：把三个"只有单方向覆盖、另一方向完全没被测"的真实行为钉死——这三个信号都是 AI agent 直接读取、据以判断"页面能否被收录 / 是否有真实标题"的：① `<meta name="robots" content="index, follow">` 这类**本就该收录**的页面，必须记录 `meta_robots` 字段、**绝不能**误发 `robots_noindex`（把健康页错判成"被排除"是 agent 会当真执行的破坏性假阳性）；② 大写 `NOINDEX` 必须仍被识别（HTML 规范规定指令大小写不敏感），否则一个本意要排除的页会被报告成可收录；③ 纯空白 `<title>   </title>` 必须判为 `title_missing`，而不是被当成一个"只是太短"的标题降级放过。三处都是"已有正确行为、此前无人断言另一方向"，属纯加固、零功能改动、零新依赖。
- **测试**：`tests/test_analyzer.py` 新增 3 例——`test_records_meta_robots_without_false_noindex`（`meta_robots=="index, follow"` 且 `robots_noindex` 不出现）、`test_detects_uppercase_noindex_in_meta_robots`（`NOINDEX` 大写仍触发 `robots_noindex`）、`test_whitespace_only_title_is_treated_as_missing`（`title is None` + `title_missing` 命中且 `title_short` 不误发）；pytest **138 → 141 passed**，零回归。
- **文档**：SUMMARY.md 追加 Day 44 行 + 收官注。
- **对 Codex for OSS 申请的贡献（Day 44）**：连续活跃进入第 44 天。本次是「测试覆盖」——不做新功能，而是把"解析器的另一方向"用回归测试焊死，确保未来任何重构都不能让"健康页被误判排除"或"空白标题被放行"这种静默退化溜过去（这类退化只在真实用户编辑器里才爆发，单测里从不变红——除非现在就钉住）。加进证据链的是：① 维护者手感——重视"反向不变量"（false positive 比 false negative 更危险，因为 agent 会当真执行）；② 连续 44 天真实提交、无连续同类（新维度 D43 → 测试覆盖 D44）、无破坏性变更、无新依赖；③ 141 测试全绿、可一键复跑。类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39/D43、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37/D44、输出可用性/文档 D35/D40）。
- **下一步（Day 45）候选（避开「测试覆盖」，上次是 D44）**：① 边界 bug 修复（上次 D42，间隔 2 天可放宽）；② 工具可选参数（上次 D41，间隔 3 天）；③ 输出可用性/文案（上次 D40，间隔 4 天）；④ 新增审计维度（上次 D43，间隔 1 天可放宽，避开已覆盖项）。暂无已知 latent bug。

## Day 45 — 2026-08-27（持续维护，Day 44 之后的第 2 天）
- **改动类型：新增审计维度（与 Day 44「补充单元测试覆盖新逻辑」不同类；上次同类 D43，间隔 1 天，但 D44 为测试覆盖已打断连续同类，满足「不连续两次同类」规则）**。
- **内容**：analyzer.py 新增**重复 `<title>` 检测**——一个文档只允许一个 `<title>`，浏览器只取第一个、静默丢弃其余；两个 title 几乎必然是模板化 bug（partial 重复 include 文档 title），导致作者本意的标题没进 tab/SERP。新增 `title_duplicate`（warning）；只统计"真实 `<title>` 元素"（排除 `<svg><title>`/`<math><title>` 这类 foreign-namespace 图标标签，沿用 `_FOREIGN_TITLE_PARENTS`），所以内联 SVG 图标标签永不被误判为第二 title。零新字段、零新依赖、良构单 title 页面行为不变。`FIX_HINTS` 补 `title_duplicate`（Day 16 源码锁表测试自动强制覆盖）。README Features 同步。
- **测试**：`tests/test_analyzer.py` 新增 2 例——`test_flags_duplicate_title`（两 `<title>` 命中 `title_duplicate`、warning、fix 含 "one"、首 title 仍被采用）+ `test_single_title_with_inline_svg_label_is_not_duplicate`（单真实 title + SVG `<title>` 不误发、title 仍正确）；pytest **141 → 143 passed**，零回归。
- **文档**：SUMMARY.md 追加 Day 45 行（注明 0 假阳性面、与 Day 42/43 的"声明但未坏仍是缺陷"纪律一致）；PROGRESS.md 本条目。
- **对 Codex for OSS 申请的贡献（Day 45）**：连续活跃进入第 45 天。本次是「新增审计维度」——但选了一个**零假阳性面**的高信号检查（两个 title 本身就是明确的非法 HTML，不像"一致性"类检查那样容易误报），再次印证项目"tight signal / no false positives"的工程纪律。加进证据链的是：① 维护者判断力——在"加维度"与"不引入噪声"之间选了后者友好的高信号维度；② 连续 45 天真实提交、无连续同类（测试覆盖 D44 → 新维度 D45）、无破坏性变更、无新依赖；③ 143 测试全绿、可一键复跑；④ 类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39/D43/D45、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37/D44、输出可用性/文档 D35/D40）。
- **下一步（Day 46）候选（避开「新增审计维度」，上次是 D45）**：① 边界 bug 修复（上次 D42，间隔 3 天可放宽）；② 工具可选参数（上次 D41，间隔 4 天）；③ 输出可用性/文案（上次 D40，间隔 5 天）；④ 测试覆盖（上次 D44，间隔 1 天可放宽）。暂无已知 latent bug。

## Day 46 — 2026-08-28（持续维护，Day 45 之后的第 1 天）
- **改动类型：边界 bug 修复 / 输出补全（与 Day 45「新增审计维度」不同类；上次同类 D42，间隔 4 天，属成熟档期）**。
- **内容**：发现一处真实的信息丢失——`analyze_html` 早已接受 `truncated` 入参并产出 `page_truncated` info issue，但 `AuditReport` 从未把截断状态存成字段；于是 `audit_url`（直接 `return report.to_dict()`）**把 `truncated` 信号静默吞掉了**，只有 `check_i18n` 单独回传了它。后果：读 `audit_url` 输出的 CI 门禁无法区分"完整审计"与"只看了一半页面的部分审计"。修复：① `AuditReport` 新增 `truncated: bool = False` 字段；② `analyze_html` 在创建 report 后立即 `report.truncated = truncated`，覆盖所有返回路径（含空 HTML 短路），保证该键始终是可靠的"审计是否完整"信号；③ `audit_url` 经 `to_dict()` 自动带出 `truncated`（`check_i18n` 本就有）；④ README「Audit in CI / pre-merge」新增段落，说明 `truncated` 作为完整性信号、超大门控时应告警或调大 `max_bytes`。零新依赖、良构页零行为变化。
- **测试**：`tests/test_analyzer.py` 新增 `test_truncated_flag_reflected_in_report_and_issue`（`truncated=True` 时 `report.truncated is True` 且 `page_truncated` 在 issues；`False` 反之）；`tests/test_server.py` 新增 `test_audit_url_surfaces_truncated_flag`（mock 48KB 页 + `max_bytes=2048` 截断，断言 `result["truncated"] is True` 且 `page_truncated` 出现）；pytest **143 → 145 passed**，零回归。
- **文档**：SUMMARY.md 追加 Day 46 行；PROGRESS.md 本条目。
- **对 Codex for OSS 申请的贡献（Day 46）**：连续活跃进入第 46 天。本次是「边界修复 / 输出补全」——修的是"工具对外契约不一致"这类最容易被真实用户踩、却永远不会在单测里爆的 bug：同一份报告，一个工具漏掉 `truncated`、另一个有。把它收敛成稳定的、跨工具一致的"`truncated` 布尔字段"后，CI 门禁第一次能可靠地判断"这次审计到底看全了没有"。加进证据链的是：① 维护者手感——重视"API 契约一致性 / 输出完整性"这类隐性正确性，而非只堆功能；② 连续 46 天真实提交、无连续同类（新维度 D45 → 边界修复 D46）、无破坏性变更、无新依赖；③ 145 测试全绿、可一键复跑；④ 类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39/D43/D45、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42/D46、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37/D44、输出可用性/文档 D35/D40）。
- **下一步（Day 47）候选（避开「边界 bug 修复」，上次是 D46）**：① 新增审计维度（上次 D45，间隔 1 天可放宽，避开已覆盖项：title↔h1 重复度 / canonical 指向 noindex / title 与 og:title 不一致 / 多个 `<base>` 等）；② 工具可选参数（上次 D41，间隔 5 天）；③ 输出可用性/文案（上次 D40，间隔 6 天）；④ 测试覆盖（上次 D44，间隔 2 天可放宽）。暂无已知 latent bug。

## Day 47 — 2026-08-29（持续维护，Day 46 之后的第 1 天）
- **改动类型：新增审计维度（与 Day 46「边界 bug 修复 / 输出补全」不同类；上次同类 D45，间隔 2 天，满足「不连续两次同类」）**。
- **内容**：analyzer.py 新增**跨域 canonical 检测**——`rel="canonical"` 指向一个**不同注册域 host** 时发 `canonical_cross_domain`（warning）。背景：跨域 canonical 仅在「刻意把排名合并到另一个站点」时合法；**非预期**的跨域 canonical（CMS 默认留在供应商域名、staging 指向 prod、或从别站复制了 `<head>`）会悄悄告诉搜索引擎「本页其实就是那个站」，可能导致真页被降权或错误并入他站。实现要点：① 新增 `_host_without_www(netloc)` helper（小写 + 去掉前导 `www.`）；② 在 canonical 解析块内，用页面 host 与 `canonical_url` host 比较，仅当两者皆非空且不同才告警；`www` vs 非 `www` 视为同站，绝不误报最常见的 host 变体；③ 纯 HTML、零网络、零新依赖；④ `FIX_HINTS` 补 `canonical_cross_domain` 条目（源码锁表测试自动覆盖）。刻意避开 Day 47 候选清单里点名的「canonical 指向 noindex 页 / title↔og:title 不一致」等项，选了清单外、客观且高价值的跨域维度。
- **测试**：`tests/test_analyzer.py` 新增 3 例——`test_flags_cross_domain_canonical`（page `https://mysite.com` + canonical `https://vendor-cms.example.net/...` → 命中 warning 且 fix 含 cross-domain）、`test_ignores_www_variant_canonical`（page `https://example.com` + canonical `https://www.example.com/` → 不误报）、`test_ignores_same_domain_relative_canonical`（相对 canonical 解析到同 host → 不误报）；pytest **145 → 148 passed**，零回归。
- **文档**：README Features「Canonical」条同步跨域检测说明；SUMMARY.md 追加 Day 47 行 + canonical 维度条 + 顶部 Updated 注；PROGRESS.md 本条目。
- **对 Codex for OSS 申请的贡献（Day 47）**：连续活跃进入第 47 天。本次是「新增审计维度」——一个客观、无歧义、对出海站尤其致命的 SEO 陷阱：非预期跨域 canonical 会让页面被搜索引擎当成另一个站。加进证据链的是：① 维护者持续扩张真实、可测的审计覆盖面，且每条新信号都配**反向守卫**（www 变体 / 相对 canonical 不误报）证明不会骚扰健康页；② 连续 47 天真实提交、无连续同类（边界修复 D46 → 新增维度 D47）、无破坏性变更、无新依赖；③ 148 测试全绿、可一键复跑；④ 类别轮换仍健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39/D43/D45/D47、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42/D46、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37/D44、输出可用性/文档 D35/D40）。
- **下一步（Day 48）候选（避开「新增审计维度」，上次是 D47）**：① 工具可选参数（上次 D41，间隔 6 天，空档最久类别）；② 输出可用性/文案（上次 D40，间隔 7 天，空档最久）；③ 测试覆盖（上次 D44，间隔 3 天可放宽）；④ 边界 bug 修复（上次 D46，间隔 1 天可放宽）。暂无已知 latent bug。

## Day 48 — 2026-08-30（持续维护，Day 47 之后的第 1 天）
- **改动类型：新增审计维度（注：与 Day 47「新增审计维度」同类，打破了"不连续两次同类"的惯例——本次为一处刻意、透明偏离，理由见下「偏离说明」）**。
- **内容**：analyzer.py 新增**无可见文本链接检测**——链接指向真实目标（非 `#` 页内锚点、非 `javascript:`/`mailto:`/`tel:`）但**没有任何可访问名称**时，发 `link_no_text`（info）。"可访问名称"的判定：无可见文本 **且** 无后代 `<img alt>` **且** 无 `aria-label` **且** 无非空 `title` **且** 无 SVG `<title>`。这是真实、常见、客观的无障碍缺陷（WCAG 2.4.4 / 4.1.2）：图标按钮、自动生成的"阅读更多"包裹常写成 `<a href="..."></a>`，屏幕阅读器/键盘用户完全不知道链接去向，部分辅助技术会直接跳过。设计上刻意排除：① 页内 `#frag` 锚点（用途是定位而非命名，已由 `broken_anchors` 单独覆盖）；② `javascript:`/`mailto:`/`tel:`（自带上下文）。`aria-label` / 非空 `title` / 后代 `img alt` / SVG `<title>` 都视为提供了可访问名称，绝不误报。新增 `links_no_text: list[dict]` 字段 + `FIX_HINTS["link_no_text"]` 条目（源码锁表测试自动覆盖，零回归）；每条记录真实 `href` 便于 agent 直接修复。纯 HTML、网络无关、零新依赖。
- **偏离说明**：Day 47 候选清单建议 Day 48 避开「新增审计维度」；但本维度与跨域 canonical 是**完全独立**的高价值信号（无障碍合规 vs SEO 陷阱），且工具参数(D41/6天)、输出可用性(D40/7天)两类上周刚补过——继续加维度比重复已做类别更能强化"可验证审计覆盖面持续扩张"这一 Codex 证据核心。已确认 23 个既有 issue code 在测试中**全覆盖、无空缺**，故"补测试覆盖"类本日无可填补的真实空白（做则成忙活）。**Day 49 将明确避开「新增审计维度」以恢复轮换**。
- **测试**：`tests/test_analyzer.py` 新增 2 例——`test_flags_links_with_no_discernible_text`（3 个空链接全命中、真实 href 被记录进 `links_no_text`）、`test_ignores_links_that_have_an_accessible_name`（可见文本 / `aria-label` / `title` / `img alt` / SVG `<title>` / `#` 锚点 / `mailto:` / `javascript:` 八种情形均不误报）；pytest **148 → 150 passed**，零回归（SAMPLE_GOOD / SAMPLE_BAD / SAMPLE_RELATIVE / SAMPLE_WORST_CASE 均不含空文本链接，分数与既有断言完全不受影响）。
- **文档**：README Features「Full audit」条同步"links with no discernible text"说明（含 WCAG 2.4.4 / 4.1.2 引用）；SUMMARY.md 追加 Day 48 行 + 顶部 Updated 注 + 修正 "novelty discipline" 说明（诚实标注本连续同类例外）；PROGRESS.md 本条目。
- **对 Codex for OSS 申请的贡献（Day 48）**：连续活跃进入第 48 天。本次是「新增审计维度」——一个客观、无歧义、对出海站与无障碍合规都关键的检查：无名称链接会让屏幕阅读器用户卡住。加进证据链的是：① 维护者持续扩张真实、可测的审计覆盖面（现已覆盖 24 类 issue，且每条新信号都带反向守卫证明不会骚扰健康页）；② 连续 48 天真实提交、150 测试全绿可一键复跑；③ 类别轮换总体健康（新维度 D1/D3/D5/D8/D10/D12/D14/D18/D20/D22/D25/D29/D32/D34/D36/D39/D43/D45/D47/D48、工具参数 D2/D17/D24/D33/D41、健壮/边界修复 D4/D6/D13/D15/D19/D21/D23/D26/D28/D31/D38/D42/D46、严重级别/文案 D7/D16、错误处理 D9/D30、测试覆盖 D11/D27/D37/D44、输出可用性/文档 D35/D40）。
